...
public class NotificationHelper {
    ...
    /**
     * Broadcast to ALL drivers using filters (better for background wake up)
     */
    public void broadcastRideRequest(String customerName, String pickup, String rideId) {
        String appId = context.getString(R.string.onesignal_app_id);
        String restApiKey = context.getString(R.string.onesignal_rest_api_key);
        ...
        try {
            ...
            // Filters to target all drivers
            JSONArray filters = new JSONArray();
            JSONObject filter = new JSONObject();
            filter.put("field", "tag");
            filter.put("key", "user_type");
            filter.put("relation", "=");
            filter.put("value", "Drivers"); // Must match what we set in initialize
            filters.put(filter);
            notificationContent.put("filters", filters);

            JSONObject contents = new JSONObject().put("en", customerName + " needs a ride from " + pickup);
            JSONObject headings = new JSONObject().put("en", "New Ride Request");
            notificationContent.put("contents", contents);
            notificationContent.put("headings", headings);
            ...
            Log.d(TAG, "Broadcasting notification: " + notificationContent.toString());
            ...
            httpClient.newCall(request).enqueue(new Callback() {
                ...
                @Override
                public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                    Log.d(TAG, "Broadcast response: " + response.code());
                    response.close();
                }
            });
        } catch (JSONException e) {
    ...
    public void sendRouteFullNotification(String driverId, String routeName) {
        // Fetch driver's notification key from Firebase first
        FirebaseDatabase.getInstance().getReference()
                .child("Users").child("Drivers").child(driverId).child("notificationKey")
                .get().addOnSuccessListener(snapshot -> {
                    if (snapshot.exists()) {
                        String notificationKey = snapshot.getValue(String.class);
                        try {
                            JSONObject data = new JSONObject();
                            data.put("type", "route_full");
                            data.put("routeName", routeName);
                            data.put("timestamp", System.currentTimeMillis());

                            sendNotification(notificationKey, "Route Full", 
                                "Your fixed route (" + routeName + ") is now fully booked!", data);
                        } catch (JSONException e) {
                            Log.e(TAG, "Error creating route full notification", e);
                        }
                    } else {
                        Log.w(TAG, "Driver has no notification key: " + driverId);
                    }
                }).addOnFailureListener(e -> Log.e(TAG, "Failed to fetch driver notification key", e));
    }
